...
app.get('/playingMovies/', function(req, res)
{
	...
	.then (response =>
	{
		...
		for (m of response.data.results)
		{
			...
			currMovie['poster'] = 'https://image.tmdb.org/t/p/original' + m.backdrop_path;
...
app.get('/mediaReel/:type/:medium', function (req, res)
{
	...
	.then (response =>
	{
		...
		for (m of response.data.results)
		{
			...
			currMedia['poster'] = (m.poster_path != null) ? ('https://image.tmdb.org/t/p/w500' + m.poster_path) : "assets/media-portrait.png";
...
app.get('/details/:type/:id', function (req, res)
{
	...
	.then (axios.spread((rDets, rVid) =>
	{
		...
		fullDetails['poster'] = (details.poster_path != null) ? ('https://image.tmdb.org/t/p/w500' + details.poster_path) : "assets/media-portrait.png";
		...
		fullDetails['vote'] = (details.vote_average != null) ? details.vote_average : 0;
...
app.get('/cast/:type/:id', function (req, res)
{
	...
	.then (response =>
	{
		...
		for (c of castData)
		{
			...
			person['headshot'] = (c.profile_path != null) ? ('https://image.tmdb.org/t/p/w500/' + c.profile_path) : 'assets/person.jpg';
			cast.push(person);
		}
		res.send(cast);
	})
...
app.get('/reviews/:type/:id', function (req, res)
{
	...
	.then (response =>
	{
		...
		for (r of revs)
		{
			...
			review['rating'] = (r.author_details.rating) ? r.author_details.rating : 0;
   ...
   app.get('/individual/:type/:id/:topic', function (req, res)
   {
	   ...
	   .then (response =>
	   {
		   ...
		   for (r of result)
		   {
			   ...
			   curr['poster'] = (r.poster_path != null) ? ('https://image.tmdb.org/t/p/w500' + r.poster_path) : 'assets/media-portrait.png';
...
function parseReslts(m)
{
	...
	output['poster'] = (m.backdrop_path != null) ? ('https://www.themoviedb.org/t/p/w780' + m.backdrop_path) : "assets/media-landscape.jpg"; 
	output['id'] = m.id;
...
function calcDate(ugly_date)
{
	...
	options = {month: 'long', day: 'numeric', year: 'numeric',  hour: 'numeric', minute: 'numeric', second: 'numeric',};
